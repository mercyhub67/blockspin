...
local function VelocityDesync()
    ...
    -- คืนค่าเสมอ ไม่ว่าจะเกิดอะไรขึ้น
    pcall(function()
        hrp.Velocity = OldVec
...
local function ResetPhysics()
    ...
    if hrp then
        hrp.CustomPhysicalProperties = PhysicalProperties.new(
            0.7, 0.3, 0.5, 0, 0  -- ค่า default ของ Roblox
        )
    end
end
...
MainTab:Toggle({
    ...
    Callback = function(Value)
        ...
        if Value then
            ...
            stopDanceAntiAim()
            ResetPhysics()  -- คืน physics กลับปกติเมื่อปิด
        end
    end
})
...
func["EnabledSnap"] = function()
    ...
    -- ขอ network ownership ก่อนบังคับ CFrame
    pcall(function() RootPart:SetNetworkOwner(Players.LocalPlayer) end)
    ...
    -- คืน network ownership เมื่อปิด
...
local function createESP(player)
    ...
    local function onCharacter(char)
        ...
        espConnection = RunService.RenderStepped:Connect(function()
            -- character ออกไปแล้วหรือตาย → ลบ GUI และ disconnect loop
            if not char.Parent or not hum or hum.Health <= 0 then
                espConnection:Disconnect()
                pcall(function() nameGui:Destroy() end)
                pcall(function() distGui:Destroy() end)
...
local function StartAutoMod()
    ...
    -- Realtime update loop ทุก frame
...
-- Auto-start ทันทีโดยไม่ต้องกด toggle
