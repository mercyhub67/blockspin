...
-- ── Anti-Antiaim State ────────────────────────────────────────
local antiAntiaimEnabled  = false   -- toggle หลัก
local aaHeadHistory       = {}      -- เก็บ history ตำแหน่งหัวของ target
local AA_HISTORY_SIZE     = 8       -- จำนวน frame ที่เก็บ
local AA_JITTER_THRESHOLD = 15      -- องศา ถ้า head หมุนเกินนี้ต่อ frame = jitter
local AA_MULT_THRESHOLD   = 40      -- studs ถ้า head ขยับเกินนี้ต่อ frame = multiplier spin
...
-- ══════════════════════════════════════════════════════════════
--  Anti-Antiaim Detection & Override
-- ══════════════════════════════════════════════════════════════

-- บันทึก history ตำแหน่ง/rotation ของหัว target ทุก Heartbeat
RunService.Heartbeat:Connect(function()
    if not antiAntiaimEnabled then return end
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character then
            local head = player.Character:FindFirstChild("Head")
            local root = player.Character:FindFirstChild("HumanoidRootPart")
            if head and root then
                aaHeadHistory[player] = aaHeadHistory[player] or {}
                table.insert(aaHeadHistory[player], {
                    time    = os.clock(),
                    headPos = head.Position,
                    rootCF  = root.CFrame,
                })
                if #aaHeadHistory[player] > AA_HISTORY_SIZE then
                    table.remove(aaHeadHistory[player], 1)
                end
            else
                aaHeadHistory[player] = nil
            end
        end
    end
end)

Players.PlayerRemoving:Connect(function(p) aaHeadHistory[p] = nil end)

-- ตรวจว่า target ใช้ jitter (หัวสั่นเปลี่ยน rotation เร็ว)
local function isJitterAntiaim(player)
    local hist = aaHeadHistory[player]
    if not hist or #hist < 3 then return false end
    local changes = 0
    for i = 2, #hist do
        local dt = hist[i].time - hist[i-1].time
        if dt > 0 then
            local prevLook = hist[i-1].rootCF.LookVector
            local curLook  = hist[i].rootCF.LookVector
            local dot = math.clamp(prevLook:Dot(curLook), -1, 1)
            local angleDeg = math.deg(math.acos(dot))
            if angleDeg > AA_JITTER_THRESHOLD then
                changes = changes + 1
            end
        end
    end
...
-- ตรวจว่า target ใช้ multiplier/spin (head เคลื่อนที่ไกลมากต่อ frame)
local function isMultiplierAntiaim(player)
    local hist = aaHeadHistory[player]
    ...
    local speed = (last.headPos - prev.headPos).Magnitude / dt
    return speed > AA_MULT_THRESHOLD
end

-- คืนค่า aim position ที่เหมาะสมตาม antiaim ที่ตรวจพบ
local function getAntiAntiaimPos(player, head, root)
    local hist = aaHeadHistory[player]

    if isMultiplierAntiaim(player) then
        -- Multiplier/spin: head spin เร็วมาก → ยิงที่ root offset แทน (ไม่โดน spin)
        return root.Position + Vector3.new(0, 1.5, 0), head
    end

    if isJitterAntiaim(player) then
        -- Jitter: ใช้ค่าเฉลี่ย head position → aim ที่หัวจริง
        if hist and #hist >= 2 then
            local sum = Vector3.new()
            for _, entry in ipairs(hist) do
                sum = sum + entry.headPos
            end
            return sum / #hist, head
        end
    end

    -- ปกติ: ใช้ predictPosition
    return predictPosition(head, root), head
end
        ...
        originalFireServer = hookfunction(SendRemote.FireServer, function(self, ...)
            ...
            if silentAimEnabled and args[2] == "shoot_gun" and aimTarget then
                ...
                if head and root and hum then
                    -- ── Anti-Antiaim Override ─────────────────────────────
                    local aimPos, aimInstance
                    if antiAntiaimEnabled then
                        aimPos, aimInstance = getAntiAntiaimPos(aimTarget, head, root)
                    else
                        aimPos    = predictPosition(head, root)
                        aimInstance = head
                    end
...
local AntiAntiaimToggle = CombatTab:Toggle({
    Title   = "Anti-Antiaim",
    Desc    = "ตรวจ Multiplier/Jitter AA แล้วบังคับ SilentAim ยิงโดน",
    Default = false,
    Callback = function(v)
        antiAntiaimEnabled = v
        aaHeadHistory = {}
        if WindUI then
            WindUI:Notify({
                Title       = v and "✅ Anti-Antiaim ON" or "❌ Anti-Antiaim OFF",
                Description = v and "ตรวจ Spin/Jitter อัตโนมัติ" or "ปิดแล้ว",
                Duration    = 3,
            })
        end
    end,
})
Config:Register("AntiAntiaim", AntiAntiaimToggle)
