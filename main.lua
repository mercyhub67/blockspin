...
local Sr = setmetatable({}, {
    __index = function(_,d)
        return cloneref(game:GetService(d))
    end
})

local function Connect(Event, Function)
	if not Event or not Function then 
		warn('Not found Event or Function')
		return 
	end

	local EventConnected = Event:Connect(Function)
	return EventConnected
end

local RunService = Sr.RunService
local ReplicatedStorage = Sr.ReplicatedStorage
...
local function IsAlive(model)
    ...
    return Humanoids.Health > 0 and true or false
end
...
local function HighVelocity(r,rce)
	...
	r.Velocity = a
	r.AssemblyLinearVelocity = b
	r.AssemblyAngularVelocity = c
	r.RotVelocity = v
	rce.Velocity = ab
	rce.AssemblyLinearVelocity = bb
	rce.AssemblyAngularVelocity = cb
	rce.RotVelocity = vb
end
...
local function Csync(r)
	...
	local FakeCFrame = CFrame.new(r.Position + Vector3.new(math.random(-1,2), math.random(-1,2), math.random(-1,2)))
...
Connect(RunService.Heartbeat, function()
	if getgenv().AntiAim then   
		...
		if HumanoidModule and not HumanoidModule:GetAttribute("HasBeenDowned") then 
			...
			HighVelocity(RootPartModule,RootPart)
...
local __index; __index = hookmetamethod(game, "__index", function(Self, Index)
	if not checkcaller() and getgenv().AntiAim and Index == "CFrame" and Self == RootPart and IsAlive(Character) and SavedHistoryCFrame and not Char.get_hum():GetAttribute("HasBeenDowned") then
